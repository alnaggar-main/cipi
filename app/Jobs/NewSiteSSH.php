...
class NewSiteSSH implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $ssh->exec('echo '.$this->server->password.' | sudo -S sudo bash newsite -dbr '.$this->server->database.' -u '.$this->site->username.' -p '.$this->site->password.' -dbp '.$this->site->database.' -d '.$this->site->domain.' -php '.$this->site->php.' -id '.$this->site->site_id.' -r '.config('app.url').' -b '.$this->site->basepath);
        $ssh->exec('echo '.$this->server->password.' | sudo -S sudo unlink newsite');
        $ssh->exec('exit');
    }
}
